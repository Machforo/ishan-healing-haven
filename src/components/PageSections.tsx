  ...
  useEffect(() => {
    const fetchGlobalSections = async () => {
      try {
        ...
        let res = await fetch(`${apiBase}/hospital/page-sections/by-url?url=${encodeURIComponent(location.pathname)}`);
        if (!res.ok && cleanPath !== location.pathname) {
          res = await fetch(`${apiBase}/hospital/page-sections/by-url?url=${encodeURIComponent(cleanPath)}`);
        }
        if (res.ok) {
